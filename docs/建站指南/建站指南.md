...
同时，在 `style.css` 中，添加以下代码，实现“VitePress”的渐变显示，以及稍微增大三行标题之间的间距。
...
修改 `cofig.mts` 中的 `sidebar`，其中 `collapsed: false` 表示可折叠但默认展开。
...
点击右下角 Date Sources -> Indices -> Create Index 创建索引，名字随意，之后要用。
